...
function Baresip(config) {
    ...
    self.run = function () {
        self.intervalObject = setInterval(function () {
            self.baresipWrapper.getCallList(function (err, callList) {
                ...
                var establishedCalls = self.findEstablishedCalls(callList);
                ...
                for (var i in establishedCalls) {
                    self.callList[establishedCalls[i].sip].status = "ESTABLISHED";
                    self.emit("established_call", establishedCalls[i]);
                }
            });
    ...
    self.findEstablishedCalls = function (newCallList) {
